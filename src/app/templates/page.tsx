...
export default function TemplatesPage() {
  ...
  useEffect(() => {
    Promise.all([getSectionTemplates(), getCategories()]).then(([data, cats]) => {
      if (!isAdmin(profile?.role)) {
        data = data.filter((t) => t.created_by === profile?.id);
      }
      setTemplates(data);
      setCategories(cats);
      setLoading(false);
  ...
  const showActions = (t: SectionTemplate) => isAdminUser || t.created_by === profile?.id;
                  ...
                  <div className="pl-4 p-3 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-medium text-sm leading-snug text-gray-900">
                        {template.title}
                        {isAdminUser && template.creator_name && (
                          <span className="inline-flex items-center gap-1 ml-2 text-[10px] font-normal text-gray-400">
